 ...
 *
 * A pick the lab stages later than the guide's tier ("LATE") is a lead, not a verdict: the guides
 * order bosses by difficulty (Deerclops before Skeletron) while the lab follows BossChecklist's
 * progression, and the miner's answer comes with its evidence (`stageSource`). Fix the miner or
 * the config when the evidence is wrong; there is no override file.
 *
 * Sources
...
const lines = [];
const lateOverrides = new Map(); // item id → earliest guide tier stage (reported, never written)
