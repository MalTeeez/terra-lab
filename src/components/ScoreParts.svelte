...
<span class="inline-flex flex-wrap items-center gap-1 text-[11px] text-dim">
  ...
  {/if}
  {#each shown as p}
    <span class="lab-part has-tip" class:neg={p.value < 0} data-tip={factorTip(p, weapon)}><span style="color:{FACTORS[factorOf(p, weapon)].color}">{p.label}</span><b>{p.value > 0 ? '+' : ''}{fmtNum(p.value)}</b></span>
  {/each}
  {#if rest.length}
