...
| `STATE_GATED` | a spawn whose `if` reads a state of the **target** — a mod tracker on `GlobalNPC`, the boss's remaining life — taken from the arm the miner read without its guard | the walk reading what *sets* that field (the Walking Cane's own stealth strike sets `caneInsanityTimer`, one branch up in the same method) | none: 10 children in the pack carry one, each a lead in `unresolved-phases` |
...
| `DRAG_STALL = 2` px/update | the speed a decelerating projectile stops being a weapon at, which is what bounds its reach instead of the asymptote `v/(1−d)` it would creep up on given for ever | the boss's own speed, which `reachOf` is not handed — it takes a projectile and a step and nothing about what is being shot at. Passing `vb` in retires it | a dragging shot on a moving dummy at increasing distances: where it stops connecting |
| `CARRIER_EXPLAINED = 0.25` hits/use | the line between "the cascade is where this carrier's damage went" and "the cascade is a gap with a number in front of it", for a projectile the game will not let damage anything | every gate under the cascade being read (`threshold` domains, spawn cadences): where the cascade is right the fallback is never reached | a never-friendly holdout weapon on a dummy, against what the model's cascade predicts for it |
...
| 2026-09-09 | four reads from a player report: `CanDamage() => false`, a join is not an else, post-hit velocity is not flight, and a child's launch speed | **146 → 149 / 295 → 296** (pre set) | calamity 0.283 → 0.276, ieor and vanilla flat | 128/141 | five complaints, four of them a miner read. **(a)** *"The Cauldron shows up as a held beam but just fires falling projectiles"* — its holdout overrides `CanDamage() => false`, the newer way to say what `friendly = false` says, and the `friendly` reader cannot see it: the thing sets `friendly = true` in `SetDefaults` like any shot and then refuses every hit at the hook. **112 projectiles** across the pack say it that way, nearly all of them holdouts and spawners (`GlassmakerHoldout`, `Catharsis Spawner`, `IllusionersReverieProj`, `DeathsAscensionRift`), and every one was billing six contact hits a second. Read only in its constant form — a `CanDamage` with a branch in it is a different fact this does not claim to have read. Cauldron 333 → 35/s, #1 magic at post-EoW → #36. **(b)** *"the Archaeologist's Toolbelt is marked a pretty good accessory when the description doesn't even say anything about the attacking part"* — and the tooltip is right. `SOTSItem.CanUseItem` guards `if (isConduitItem && !ConduitBelt) return false;`, and `openElse` read the block after that early return as the *else* of `ConduitBelt`, hanging every later proc in the method — a shrimp laser and the Wishing Star — on the belt. But the `isConduitItem` test jumps past the return as well, so the block is a **join**, not a negation: `openElse` now refuses when any jump reaches the else start from outside the if it is closing. It also no longer requires the closing region to be the *innermost* one, which is what let (c) work. Toolbelt 15 → 0 points, #8 → #279 of the stage's accessories; 36 weapons move on the region fix alone, in both directions, Storm Maiden's Retribution 878k → 169k the largest. **(c)** *"Meteor Fist is shown as 0 damage but does very good damage"* — 0 because `velocity.X *= 0.9f; velocity.Y += 0.2f` was read as its drag and its arc, dropping it 4410 px over the 60 px it was given. Both writes sit inside `if (numHits != 0)`: it is a **wire-guided fist that steers onto the cursor at up to 22.5 px/tick and only falls once it has hit something**. `numHits` is now a flag like `stealthStrike`, so the arm it guards is tagged and a velocity write inside it is not read as flight. Two projectiles carry it — the fist, and Calamity's Mana-charged Coral, whose 0.8 gravity is likewise what it does *after* it sticks (through `HasStuck => numHits > 0`, which the walk follows). Meteor Fist 0 → 35/s, unranked → #17 of 61 rogue. **(d)** The Cauldron's cascade then read **zero**: `landing` guesses 8 px/tick for every spawned projectile and the lava bomb is a mortar fired straight up at 18, so its whole range came out at 126 px of the 180 it needed and the weapon was left standing on the holdout's phantom hit. A `NewProjectile` velocity built as *direction × number* now records that number (`children[].speed`, into the existing `absVelocity` phase field). **263 of 2318 child spawns** gain one, median 10 px/tick. Narrowed to a direction the walk saw **multiplied**: a bare `Vector2.UnitY` says nothing about how fast the thing travels because the child's own AI moves it, and trusting it dropped 71 more spawns to 1 px/tick — vanilla's Lunar Flare among them. **(e)** *"the Wishing Star is marked a bit low"* — it was scored off the wrong branch entirely. `WishingStar.IsAlternate` is `UniqueVisionNumber % 8 == 7`; the per-attack star in `CanUseItem` is behind it, and what seven players in eight get is in **`OnConsumeMana`** — a 100-damage star every 100 mana of magic cast — which no hook list reached. `SPEND_HOOKS` reads it on a `ModPlayer`, the helper's damage argument is taken as its **one integer parameter** (a flat 100 as readily as the sentinel), `gated` is scoped to hooks that run for every item (a spend hook is not choosing between weapons), and `SPEND_EVERY` prices it at one payout per five seconds rather than per attack. 7.5 → 15, at the proc cap, with the arithmetic on the card: 60 DPS against a typical 111 DPS weapon. **Not a defect: the Space Gun.** It already outranks the Vilethorn at every stage and target mode the pack has them both in (post-EoW 59 vs 59 with the Vilethorn a hair up, next stage 50 vs 37, in a crowd 107 vs 62); the two reasons that are DPS are modelled and the third is not one — 21.2 mana/s against 21.4, the faster fire rate eating the cheaper cast. What separates them is the Vilethorn's `archetype uptime 85%` and its six contact hits a second, both standing assumptions in `unresolved-phases`, and an in-game reading is what would settle it. Gate waived: the Cauldron is a guide pick in `ieor|magic` and `calamity|magic` and its fall is most of the 12 rows; three more are `rankable` growing as Meteor Fist and Alphorn enter the pool from 0. |
| 2026-09-09 | a blast hits everything inside it, and a projectile that sets its own speed is not travelling at its `shootSpeed` | **149 → 148 / 296 → 296** (pre set), calamity top-3 +4 and ieor +2 over all tiers | calamity 0.239 → 0.241, ieor and vanilla flat | 128/141 | the follow-up to the row above: *"the Meteor Fist on stealth strike is still not nearly high enough for multi target given its damage and explosion size"* — and both halves of that were a miner read. **(a)** Its stealth strike drops a meteorite whose `OnKill` is `ExpandHitboxBy(300); penetrate = -1; usesLocalNPCImmunity = true; localNPCHitCooldown = -1; Damage();` — the standard way a mod writes an explosion, and what it says is **one hit on every body inside the blast**. None of it was read: `penetrate` is only taken from `SetDefaults` and the AI phase, so the blast counted for a single hit in a crowd of six. `penetrate = -1` in the *kill* phase is now `blast`, a fact kept apart from the flight's `pen` because a bomb that pierces nothing on the way in still levels what it lands in — **54 projectiles** carry it, every one a mortar, grenade or bomb (the Cauldron's own Smauldron among them). The model multiplies a blast child's hits by the bodies its radius covers, `1 + explode / 2·bossWidth` bounded by the crowd, which is 1 on a single target and needs no condition of its own. **(b)** The fist itself was still flying at the item's `shootSpeed` of 4 while its AI steers it onto the cursor at up to 22.5 — 105 ticks of lead over 420 px, a ×0.24 that was most of what was left. `velocity = <direction> * k` in an AI is the projectile stating its own speed, and the `dir` value built for the child-launch read already carries it; **107 projectiles** now record a `cruise`, median 13.5 px/update (SOTS's Earthen Spirit snapping to its target at 50, Calamity's Abyss Blade at 25). `speedOf` takes the **middle** of launch and cruise, the same reading the miner's `Clamp` and `Lerp` handlers already get for a ramp the walk cannot follow. A `SPEED_FLOOR` of 3 px/update guards both this and the child-launch read: `dir * item.shootSpeed * 0.55f` is the holdout idiom and keeping the 0.55 had the Cauldron's holdout travelling half a pixel a tick, while vanilla's Lunar Flare is spawned along `(0, 1)` and then falls out of the sky on its own aiStyle — under the floor the model's own stand-in is the better answer than a number that is precisely the wrong one, and it takes Northern Light back from 5709 to its 12575. Meteor Fist **35 → 60/s single target (#17 → #10 of 61 rogue) and 21 → 55 in a crowd (#30 → #10)**. 67 weapons move, median ×1.08, 51 up and 16 down; Earthen Spirit Staff ×5.1, Abyss Blade ×2.2, Exoblade ×1.8, nothing cut below ×0.67. `calamity|stealth` top-3 +2 — the rogue strike family is what both halves were about. Waived on the same 13 rows as the pass above, unchanged by this one. |
| 2026-09-09 | `localNPCHitCooldown = -1` bounds a projectile's **repeats**, not its **pass** | **148 / 296** (pre set), calamity top-3 +4 and ieor +2 over all tiers | calamity 0.239 → 0.241, ieor 0.136 → 0.137, vanilla 0.276 → 0.274 | 128/141 | the third report on the same weapon, and the sharpest: *"it still feels too weak against multi-target, given its piercing — the Ashen Stalactite or Mothwing Dagger both have way less or zero pierce and are still ranked above it"*. `hitsPerProjectile` opened with `if (p.local < 0) return { hits: 1 }`, and the comment under it said what the mistake was without noticing it: *"the projectile may hit a given NPC once and never again, **whatever its pierce says**"*. A negative hit cooldown is a bound on hitting **the same body** twice. It says nothing about the next body along, which is the whole of what pierce is for — and the early return sat in front of every crowd term the function has (`bodies`, `CROWD_SWEEP`, the `spread` flag that waives `CROWD_WASTE`), so a once-per-NPC projectile came out of a crowd of six with one hit and a ×0.6 for ignoring the other five. **409 of the pool's 446** once-per-NPC projectiles pierce: Meteor Fist's pen 2, the Hellfire Arrow, God Slayer Slug, Terraprisma, every Calamity flame. The bound moves onto `imm` instead — an infinite window makes `repeats` zero and leaves the pass alone — which is the same statement in the place the arithmetic can use it. **Single-target scoring is unchanged by construction**: with one body `bodies` is 1, `total` is 1 and the function returns at its `hits <= 1.001` exit, the same 1 it used to. Meteor Fist in a crowd **55 → 83/s, #10 → #3 of 61 rogue**, now above the Ashen Stalactite (pen 1) and the Mothwing Dagger (`spent`, so one hit whatever its infinite pierce says) — which is the comparison that was made. 291 weapons move in multi-target against the start of the day, median ×1.14, 213 up and 78 down; Bladecrest Oathsword ×7.6 and Exalted Oathblade ×7.3 were reading 7 and 37 DPS in a crowd. Against single targets 39 move, all up, median ×1.19, none above ×1.7. Gate: `calamity` top-8 −4, four sections each losing one, against `vanilla` Pre-Plantera melee and ranged multi-target each gaining recall@K and top-8 — 17 section rows in both directions. Kept on the mechanical story: the rule was reading a fact that is not in the game. |
| 2026-09-20 | a spawn gated on the **target's** state is not free, a holdout that hits once is a swipe, rogue pays more for contact range, and Deerclops is fought before Skeletron | calamity top-3 114 → 113, top-8 213 → **215**; vanilla top-3 114 → **115** | calamity 0.241, ieor 0.147 → **0.149** (`ieor|rogue` 0.102 → **0.115**), vanilla 0.276 → 0.277 | 134/141 | a player report: *"Why is the Walking Cane ranked so high after deerclops? It is basically only true melee which is horrible for rogue, and doesnt even do much damage"* — 298/s and #1 of 80 rogue weapons at its own stage, twice the Shinobi Blade behind it. Four things, three of them model. **(a) The shadow hands were free.** `WalkingCaneProjectile.OnHitNPC` sets `CalamityGlobalNPC.caneInsanityTimer = 600` **on a stealth strike and returns**; every *other* hit spawns the shadow hand only while that timer runs. The miner reads the arm without its guard, `spawnPhases` files the child under region `top`, and the `spam` grade — which by construction never throws a strike — was paid a hand on all 3.75 swings a second, **122 of its 298 DPS**. `STATE_GATED` prices a branch whose condition reads the target's own state at 15%, the same share `score.js` gives a state-gated stat and for the same reason: something outside the grade has to put the boss there. Ten children in the pack carry one, and the other nine read the same way — `NPC.life` gates are a killing blow (Sparkling Empress spawns its shards only when the hit *killed*: `if (npc.life > 0) return`) or an execute threshold, neither of which holds on every hit of a boss fight. A one-sided guard on the player's own aim, owner or facing keeps its full share; only the target's state reads this way. **(b) `REACH.held` is a wall of thorns, and the cane is a swipe.** The 2026-09-04 pass gave a held `TrueMeleeDamageClass` blade contact reach and left everything else at 180 px; the cane's holdout is `RogueDamageClass`, so it fought at 180 of the 220 a rogue wants for a ×0.95. Its `localNPCHitCooldown` is **-1** — it hits a body once and never again, which is a swing and not something kept on the boss, and `contactPhase` already says exactly that about its clock. A held projectile with that window now reaches `REACH.truemelee`. Four weapons in the pack: the cane, Skytide Dragoon, Promise of Demise, The Only Thing I Know For Real. **(c) `RISK.rogue` 0.75 → 0.6.** The class preference the report is actually about: at point-blank a rogue kept three quarters of its DPS, where the class has no armour to stand there in and its own stealth grade wants 420 px. It only bites weapons a `REACH` drags in — a thrown dagger engages where it likes and pays nothing. **(d) Deerclops before Skeletron.** `progression.json` took BossChecklist's vanilla scale literally (Skeletron 5, Deerclops 6) and nothing plays that way: the Deer Thing is craftable the moment there is a Snow biome, and Calamity's own class-setup guide puts its buff advice in the *pre-Skeletron* section. 4.75, so the order reads Deerclops → Skeletron → Illusionist (5.2), and Deerclops drops are available a tier earlier. Walking Cane **298 → 145/s**, still its stage's best rogue weapon but by 17% instead of 100%. Gate, waived: `calamity|rogue` top-8 **+2** and `ieor|rogue` MRR **+13%** against `calamity|rogue` top-3 −1, `ieor|rogue` recall@K −1 (a section whose best pick the lab ranks #15 either way), `vanilla|magic` top-8 −1 / recall −2 and `calamity|magic` top-8 −1. The two magic rows are (a) on Sparkling Empress and the vanilla ones are (d): the vanilla and BossChecklist reading of where Deerclops belongs is the one the guides were written against, and it is the reading the report rejects. `data/guide-baseline.json` is **not** re-frozen — the tree carries uncommitted miner work whose drift (`ieor|summon` +11 top-3, `ieor|rogue` −2) is not this pass's to waive. |
| 2026-09-20 | a swipe is graded as the blade it is, not as the beam its archetype says | calamity top-8 215 → 214, top-3 113; vanilla top-3 **115** | calamity 0.241, `calamity|rogue` 0.227 → 0.225, `ieor|rogue` 0.102 → **0.118** | 135/141 | the follow-up on the same weapon: *"yeah thats still not acceptable — it should at max be rank 10 there"*, the cane still #1 of its stage's rogue pool at 145/s after the pass above. The three terms it paid were the archetype's `uptime` 0.85, the risk band, and nothing else: **no landing factor at all**. Every thrown rogue weapon pays for getting there — Mycoroot's ×0.71 lead and ×0.84 shared immunity window — and the cane was credited with connecting on every one of 3.75 thrusts a second. The IL says what it is: `WalkingCaneProjectile` extends `BaseSpearProjectile` (a thrust with a reelback, `TravelSpeed` 35) and overrides `Colliding` to a **line from the player's centre**, 36 px wide, out along the thrust. It is a stick. The model already prices a stick — for the swords that swing themselves — as `bladeCoverage` (the cube of relative reach, which puts a 64 px knife at 26 % of a broadsword's contact) times `bladeLanding` (whether a swing already aimed at the boss connects while it drifts, edge band included). A swipe never reached either, because `trueMelee` is `!item.noMelee` and here the damage leaves in a projectile — so the `swipe` predicate from the pass above (a `held` projectile whose immunity window is at or below zero: it hits a body once and never again) now takes those two terms **in place of** `uptime`. No new constant: it is the blade machinery applied to a blade the `noMelee` test was hiding. At 80 px that is 0.512 × 0.30 ≈ **0.19**, against the 0.275 a 100 px broadsword pays, which is the cube rule saying an 80 px stick is 70 % of a broadsword. Walking Cane **145 → 31/s, #1 → #28 of 68** at Deerclops and #3 → #22/42 in `calamity|pre-wof|rogue`; the other three swipes in the pack move with it (Skytide Dragoon 81/s, Promise of Demise 66/s, The Only Thing I Know For Real). Gate, waived: `calamity|rogue` top-8 −1 and `calamity|spam` top-3 −1 / top-8 −1, all three the cane itself — the Calamity guide *does* list it at pre-wof and the lab now puts it 22nd there. That is the disagreement the report is: a guide listing a weapon is not the same as the weapon being worth a slot, and the player who fought the boss is the arbiter the miner cannot be. `RISK.rogue` is left at the 0.6 the pass above set: without it the cane lands at #25 instead of #28 and `calamity|rogue` MRR is 0.219 rather than 0.225, against `ieor|rogue` recall@K one lower — near enough a wash, and 0.6 is the reading the report states. One number to flip if that ever reads wrong. |
